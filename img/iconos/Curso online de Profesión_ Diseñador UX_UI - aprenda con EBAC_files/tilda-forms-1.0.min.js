...
(function ($) {
  ...
  window.tildaForm = {
    ...
    arValidateErrors: {
      'EN': {
        ...
        'namerus': 'Por favor, ingresa un nombre válido',
        'nameeng': 'Por favor, ingresa un nombre válido',
        'string': 'Estos simbolos no son válidos, solo puedes usar letras, números y signos de puntuación',
        ...
        'emptyfill': 'Antes de continuar llena los campos obligatorios',
        'chosevalue': 'Por favor, selecciona una opción de las siguentes direcciones'
      },
      'RU': {
        'email': 'Por favor, ingresa un e-mail válido',
        'url': 'Por favor, ingresa una URL válida',
  ...
  $(document).ready(function () {
    ...
    window.tildaForm.successEnd = function ($jform, successurl, successcallback) {
      console.info(`successurl`, successurl);
      if ($jform.find('.js-successbox').length > 0) {
        if ($jform.find('.js-successbox').text() == '') {
          var arMessage = window.tildaForm.arMessages[window.tildaBrowserLang] || {};
          if (arMessage.success) {
      ...
      $jform.addClass('js-send-form-success');
      if (false && successcallback && successcallback.length > 0) {
    ...
    window.tildaForm.send = function ($jform, btnformsubmit, formtype, formskey) {
      ...
      if (formtype == 2 || (!formtype && formskey > '')) {
        ...
        $b2bInput = $jform.find('input[name=b2b]');
        if ($b2bInput.length > 0) {
          console.info(`$b2bInput.val()`, $b2bInput.val());
          $b2bForm = true;
        } else {
          console.info(`$b2bInput.val()`, false);
          $b2bForm = false;
        }

        var $b2bChallengesForm = false;

        var $b2bChallengesInput = $jform.find('input[name=source_id]');
        if ($b2bChallengesInput.length > 0) {
          if ($b2bChallengesInput.val() == '10501') {
            $b2bChallengesForm = true;
          }
        }
        ...
        var $awsInput;
        /*
        $awsInput = $jform.find('input[name=aws]');
        if ($awsInput.length > 0) {
          // console.info(`$awsInput.val()`, $awsInput.val());
          $awsForm = true;
        } else {
          // console.info(`$awsInput.val()`, false);
          $awsForm = false;
        }
        */
        // Захардкодил 2022-07-22
        $awsForm = true;

        var $awsWebinarsForm = false;

        var $awsSourceInput = $jform.find('input[name=source_id]');
        if ($awsSourceInput.length > 0) {
          if ($awsSourceInput.val() == '1') {
            $awsWebinarsForm = true;
          }
        }
        ...
        var grecaptcha_jwt = null;

        try {
          if (window && window.ebac && window.ebac.grecaptchaJWT) {
            grecaptcha_jwt = window.ebac.grecaptchaJWT;
          } else if (window && window.ebac && window.ebac.grecaptcha_jwt) {
            grecaptcha_jwt = window.ebac.grecaptcha_jwt;
          }
        } catch (e) {
          console.error(e);
        }
        ...
        if (grecaptcha_jwt) {
          $inputElem = $jform.find('input[name=grecaptcha_jwt]');
          if (!$inputElem || $inputElem.length == 0) {
            $jform.append('<input type="hidden" name="grecaptcha_jwt" value="">');
            $inputElem = $jform.find('input[name=grecaptcha_jwt]');
          }
          if ($inputElem.length > 0) {
            $inputElem.val(grecaptcha_jwt);
          }
        }
        ...
        try {
          ...
          var campaignCookiesJson = getCookiesByPattern(/^_vis_opt_exp_\d+_combi$/);

          if (campaignCookiesJson && Object.keys(campaignCookiesJson).length > 0) {
            var $ebacABTestInputElem = $jform.find('input[name=vwo_ab]');
            if (!$ebacABTestInputElem || $ebacABTestInputElem.length == 0) {
              $jform.append('<input type="hidden" name="vwo_ab" value="">');
              $ebacABTestInputElem = $jform.find('input[name=vwo_ab]');
            }
            if ($ebacABTestInputElem.length > 0) {
              $ebacABTestInputElem.val(campaignCookiesJson);
            }
          }
        ...
        try {
          function detectOS() {
            ...
            if (/windows phone/i.test(userAgent) || /win(dows )?nt/i.test(userAgent) || /win(dows )?xp/i.test(userAgent)) {
              return "windows";
            }
          ...
          $inputElem = $jform.find('input[name=os]');
          if (!$inputElem || $inputElem.length == 0) {
            $jform.append('<input type="hidden" name="os" value="">');
            $inputElem = $jform.find('input[name=os]');
          }
          if ($inputElem.length > 0) {
            $inputElem.val(os);
        ...
        $formurl = 'https://ebac.mx/api/subscriptions/create';

        var $awsNoCourseInput = $jform.find('input[name=no_course_form]');
        var $awsNoCourseForm = false;
        if ($awsNoCourseInput.length > 0) {
          if ($awsNoCourseInput.val() == 'true') {
            $awsNoCourseForm = true;
          }
        }

        if ($awsWebinarsForm) {
          $formurl = 'https://d9th96quea.execute-api.sa-east-1.amazonaws.com/Prod/mx/subscribe/webinar';
        } else if ($b2bChallengesForm) {
          $formurl = 'https://d9th96quea.execute-api.sa-east-1.amazonaws.com/Prod/mx/b2b-challenges/register';
        } else if ($b2bForm) {
          $formurl = 'https://d9th96quea.execute-api.sa-east-1.amazonaws.com/Prod/mx/b2b/register';
        } else if ($awsNoCourseForm) {
          $formurl = 'https://d9th96quea.execute-api.sa-east-1.amazonaws.com/Prod/mx/subscribe/no-course';
        } else if ($awsForm) {
          $formurl = 'https://d9th96quea.execute-api.sa-east-1.amazonaws.com/Prod/mx/subscribe/course';

          $inputElem = $jform.find('input[name=source_id]');
          if ($inputElem && $inputElem.length > 0) {
            var $inputElemValue = $jform.find('input[name=source_id]').val();
            if ($inputElemValue && $inputElemValue.length > 0 && $inputElemValue < 10000) {
              $jform.find('input[name=source_id]').val(parseInt($inputElemValue) + 10000);
            }
          }
        }
        ...
        if ($awsWebinarsForm) {
          ...
          var $phoneInput = $jform.find('input[name=Phone]');
          var $scheduleEventIdInput = $jform.find('input[name=schedule_id]');

          var $emailInputValue = null;
          var $phoneInputValue = null;
          var $scheduleEventIdInputValue = null;
          ...
          if ($phoneInput.length > 0 && $phoneInput.val()) {
            ...
            if ($phoneInputValue) {
              $phoneInputValue = $phoneInputValue.replace(/[^+\d]+/gi, '');
            }
          }
          if ($scheduleEventIdInput.length > 0 && $scheduleEventIdInput.val()) {
            $scheduleEventIdInputValue = $scheduleEventIdInput.val()
          }

          if (dataLayer) {
            dataLayer.push({
              event: 'webinarLead', // Константа
              eventCategory: 'subscription', // Константа
              user_email: $emailInputValue,
              user_phone: $phoneInputValue,
              schedule_event_name: null, // Отсутствует в Тильде
              schedule_event_id: parseInt($scheduleEventIdInputValue), // ID вебинара
              schedule_event_slug: null, // Код вебинара, иногда есть
              schedule_event_type: null, // Отсутствует в Тильде
              schedule_event_direction: null, // Отсутствует в Тильде
              schedule_event_form_type: 'tilda', // Тип формы, раньше было old и new
            });
          }
        }

        console.info('$formurl', $formurl);
        console.info('$b2bChallengesForm', $b2bChallengesForm);
        console.info('$b2bForm', $b2bForm);
        console.info('$awsForm', $awsForm);
        console.info('$awsNoCourseForm', $awsNoCourseForm);
        ...
        $.ajax({
          ...
          xhrFields: { withCredentials: !1 },
          success: function (json) {
            //console.info('form json', json);

            var context = json.context;
            var arn = false;
            try {
              console.info('json && json.executionArn', json && json.executionArn)
              if (json && json.executionArn) {
                context = json.executionArn.split(':').at(-1)
                arn = true;
              }
            } catch (e) {
              console.info('json && json.executionArn', e.message)
            }

            var successurl = $jform.data('success-url') + '?context=' + context + '&aws=' + arn;
            var successcallback = $jform.data('success-callback');
